...
import { Text, LinkIcon, WalletButton, StakeIcon, WithdrawIcon, ConnectButton } from "components";
import { Scrollbars } from "react-custom-scrollbars";
import { useWallet } from "@solana/wallet-adapter-react";
import { useDarkMode } from "providers/theme";
import { getPoolConfigByFarmKey } from "constants/deployConfigV2";
...
const StakePanel = (props: IStakePanelProps): ReactElement => {
  const { connected: isConnectedWallet } = useWallet();
  const { isDark } = useDarkMode();
  const classes = useStyles(props);
  const { address, onClose, onConfirm } = props;

  const pool = getPoolConfigByFarmKey(address);
  ...
  return (
    <Box className={classes.paper}>
      ...
      <Box>
        <ConnectButton className={classes.btn} fullWidth onClick={() => onConfirm()}>
          Confirm Withdraw
        </ConnectButton>
      </Box>
    </Box>
